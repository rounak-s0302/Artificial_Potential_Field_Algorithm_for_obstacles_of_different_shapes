...
ylim([0 200]);
zlim([0 100]);
radius = [6;7;9;7;5;5;6;4;9;7;5]; % Radius of the buildings);
% xxx = [120 120 150 120];
% yyy = [60 80 70 60];
% zzz = [60 60 60 60];
% plot(xxx, yyy, zzz);
...
%1 obstacle triangle
% v1=[120 60 60]; v2=[120 80 60]; v3=[150 70 60];
% 
...
% fill(xx, yy, 'r')
% %2 obstacle triangle
% v1=[20 70 60]; v2=[20 90 60]; v3=[40 80 60];
...
% fill(xx, yy, 'r')
% %3 obstacle triangle
% v1=[75 55 60]; v2=[75 65 60]; v3=[85 60 60];
% 
% triangle = [v1(:), v2(:), v3(:), v1(:)];
% plot3(triangle(1, :), triangle(2, :), triangle(3, :))
% xx = [75 75 85];
...
% fill(xx, yy, 'r')
% %4 obstacle triangle
% v1=[57 15 60]; v2=[57 25 60]; v3=[65 20 60];
% 
% triangle = [v1(:), v2(:), v3(:), v1(:)];
% plot3(triangle(1, :), triangle(2, :), triangle(3, :))
% xx = [57 57 65];
...
%1 rectangular obstacle
plot3([20 20 40 40],[75 85 85 75],[60 60 60 60]);
fill([20 20 40 40],[75 85 85 75], 'r');

%2 rectangular obstacle
plot3([75 75 85 85],[55 65 65 55],[60 60 60 60]);
fill([75 75 85 85],[55 65 65 55], 'r');

%3 rectangular obstacle
plot3([55 55 65 65],[15 25 25 15],[60 60 60 60]);
fill([55 55 65 65],[15 25 25 15], 'r');
...
grid on;
text(start(1,1)-1, start(1,2), start(1,3)+2,"Robot Initial Position")
...
iteration = 800;                         %Iterations count
...
Urep = 0;
figure(1)
title('Path Planning of a Robot')
for i=1:iteration
    p_Fr = 0;
    ...
    for k = 1: length(obstacles)
        % Measuring the horizantal distance between Robot and centre axis of the building 
        ...
        % Threshold value to judge whether the Robot near to obstacle or not?
        zeta = 3.5*radius(k,1);
        n = 2;
        if rou<=zeta
            % Flag - that tells the Robot to move in xy plane
            flag = 1;
    ...
    % Plotting the Robot position in real time
